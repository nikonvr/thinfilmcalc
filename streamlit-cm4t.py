...
def plot_stack_structure(res, params, layer_multipliers):
    ...
    num_layers = len(layer_multipliers)
    layer_labels = [f"Layer {num_layers - i}" for i in range(num_layers)]

    colors = ['lightblue' if np.real(n) == np.real(nH_complex) else 'lightcoral' for n in indices_reversed]

    bars = ax.barh(range(num_layers), thicknesses_reversed, align='center', color=colors, edgecolor='black')
    ...
    fontsize = max(7, 10 - num_layers // 4)
    ...
    substrate_line_y = num_layers - 0.5
    ax.axhline(substrate_line_y, color='gray', linestyle='--', linewidth=2)
    ax.text(np.mean(ax.get_xlim()) if thicknesses_reversed.size > 0 else 0.5,
            substrate_line_y + 0.1,
            'Substrate',
            va='bottom',
            ha='center',
            color='gray',
